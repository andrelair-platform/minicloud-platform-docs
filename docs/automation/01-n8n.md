...
These are **not competitors** — they are three *different shapes* of orchestration, chosen by the
nature of the work, not preference. Picking the wrong one is the common mistake.
...
**Litmus test:** reactive glue → **n8n**; a transaction that must survive failure and complete →
**Temporal**; a scheduled pipeline over data → **Airflow**. Two orchestrators are live (n8n +
Temporal, each with a distinct role); the third is deferred until its use case (scheduled data
pipelines) becomes real — see [Temporal](./temporal) and [Airflow](./airflow).
